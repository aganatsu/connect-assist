                    ...
                    return (
                      <div className="divide-y divide-border/40">
                        {latestDetailsClean.map((sig: any, i: number) => {
                          ...
                          const statusLabel = s === "limit_order_from_watchlist" || s === "zone_setup_from_watchlist" ? "🔍📋 ZONE+WL" : s === "limit_order_placed" || s === "zone_setup_active" ? "🔍 ZONE SETUP" : s === "trade_placed_from_watchlist" ? "📋 WATCHLIST" : s === "trade_placed_at_zone" ? "✅ PLACED@ZONE" : s === "trade_placed" ? "✅ PLACED" : s === "rejected" ? "REJECTED" : s === "below_threshold" ? "SKIP" : s === "watching_zone" ? "⏳ WATCHING" : s === "waiting_for_sweep" ? "⏳ SWEEP WAIT" : s === "paused" ? "⏸ PAUSED" : s === "no_direction" ? "— NO DIR" : s === "zone_setup_rejected_orientation" ? "⚠ ORIENT" : s === "staged_new" ? "\u2B50 NEW WATCH" : s === "staged_watching" ? "\uD83D\uDC41 WATCHING" : s === "staged_confirming" ? "\u23F3 CONFIRMING" : s === "staged_invalidated" ? "\u274C INVALIDATED" : s?.startsWith("skipped_") ? "SKIPPED" : s?.toUpperCase() || "\u2014";
                          const statusColor = s === "limit_order_from_watchlist" || s === "zone_setup_from_watchlist" ? "text-tier3 bg-purple-500/10 border-purple-500/30" : s === "limit_order_placed" || s === "zone_setup_active" ? "text-info-c bg-badge-info border-blue-500/30" : s === "trade_placed_from_watchlist" ? "text-cyan-400 bg-cyan-500/10 border-cyan-500/30" : s?.startsWith("trade_placed") ? "text-success bg-success/10 border-success/30" : s === "rejected" || s === "zone_setup_rejected_orientation" ? "text-destructive bg-destructive/10 border-destructive/30" : s === "watching_zone" ? "text-warn bg-badge-warn border-amber-500/30" : s === "waiting_for_sweep" ? "text-purple-400 bg-purple-500/10 border-purple-500/30" : s === "paused" || s === "no_direction" ? "text-zinc-400 bg-zinc-500/10 border-zinc-500/30" : s?.startsWith("staged_") ? "text-warn bg-badge-warn border-amber-500/30" : "text-muted-foreground bg-muted/20 border-border";
...
function ScanSignalDetail({ signal: d }: { signal: any }) {
  ...
  const statusLabel = d.status === "limit_order_from_watchlist" || d.status === "zone_setup_from_watchlist" ? "🔍📋 ZONE+WL" : d.status === "limit_order_placed" || d.status === "zone_setup_active" ? "🔍 ZONE SETUP" : d.status === "trade_placed_from_watchlist" ? "📋 WATCHLIST" : d.status === "trade_placed_at_zone" ? "✅ PLACED@ZONE" : d.status === "trade_placed" ? "✅ PLACED" : d.status === "waiting_for_sweep" ? "⏳ SWEEP WAIT" : d.status === "watching_zone" ? "⏳ WATCHING" : d.status === "paused" ? "⏸ PAUSED" : d.status === "no_direction" ? "— NO DIR" : d.status === "rejected" ? "REJECTED" : d.status === "below_threshold" ? "SKIP" : d.status?.startsWith("skipped_") ? "SKIPPED" : d.status?.startsWith("staged_") ? "⏳ STAGED" : d.status?.toUpperCase() || "—";
  const statusColor = d.status === "limit_order_from_watchlist" || d.status === "zone_setup_from_watchlist" ? "text-tier3 bg-purple-500/10 border-purple-500/30" : d.status === "limit_order_placed" || d.status === "zone_setup_active" ? "text-info-c bg-badge-info border-blue-500/30" : d.status === "trade_placed_from_watchlist" ? "text-cyan-400 bg-cyan-500/10 border-cyan-500/30" : d.status?.startsWith("trade_placed") ? "text-success bg-success/10 border-success/30" : d.status === "rejected" ? "text-destructive bg-destructive/10 border-destructive/30" : d.status === "waiting_for_sweep" ? "text-purple-400 bg-purple-500/10 border-purple-500/30" : d.status === "paused" || d.status === "no_direction" ? "text-zinc-400 bg-zinc-500/10 border-zinc-500/30" : "text-muted-foreground bg-muted/20 border-border";
...
function ScanDetailInline({ signal: d }: { signal: any }) {
  const statusLabel = d.status === "limit_order_from_watchlist" || d.status === "zone_setup_from_watchlist" ? "🔍📋 ZONE+WL" : d.status === "limit_order_placed" || d.status === "zone_setup_active" ? "🔍 ZONE SETUP" : d.status === "trade_placed_from_watchlist" ? "📋 WATCHLIST" : d.status === "trade_placed_at_zone" ? "✅ PLACED@ZONE" : d.status === "trade_placed" ? "✅ PLACED" : d.status === "waiting_for_sweep" ? "⏳ SWEEP WAIT" : d.status === "watching_zone" ? "⏳ WATCHING" : d.status === "paused" ? "⏸ PAUSED" : d.status === "no_direction" ? "— NO DIR" : d.status === "rejected" ? "REJECTED" : d.status === "below_threshold" ? "SKIP" : d.status?.startsWith("skipped_") ? "SKIPPED" : d.status?.startsWith("staged_") ? "⏳ STAGED" : d.status?.toUpperCase() || "—";
  const statusColor = d.status === "limit_order_from_watchlist" || d.status === "zone_setup_from_watchlist" ? "text-tier3" : d.status === "limit_order_placed" || d.status === "zone_setup_active" ? "text-info-c" : d.status === "trade_placed_from_watchlist" ? "text-cyan-400" : d.status?.startsWith("trade_placed") ? "text-success" : d.status === "rejected" ? "text-destructive" : d.status === "waiting_for_sweep" ? "text-purple-400" : d.status === "paused" || d.status === "no_direction" ? "text-zinc-400" : "text-muted-foreground";
