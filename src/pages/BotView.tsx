                    ...
                    return (
                      <div className="divide-y divide-border/40">
                        {latestDetailsClean.map((sig: any, i: number) => {
                          ...
                          const statusLabel = s === "limit_order_from_watchlist" || s === "zone_setup_from_watchlist" ? "🔍📋 ZONE+WL" : s === "limit_order_placed" || s === "zone_setup_active" ? "🔍 ZONE SETUP" : s === "trade_placed_from_watchlist" ? "📋 WATCHLIST" : s === "trade_placed_at_zone" ? "✅ PLACED@ZONE" : s === "trade_placed" ? "✅ PLACED" : s === "rejected" ? "REJECTED" : s === "below_threshold" ? "SKIP" : s === "watching_zone" ? "⏳ WATCHING" : s === "waiting_for_sweep" ? "⏳ SWEEP WAIT" : s === "waiting_for_reconfirmation" ? "⏳ RECONFIRM" : s === "paused" ? "⏸ PAUSED" : s === "no_direction" ? "— NO DIR" : s === "zone_setup_rejected_orientation" ? "⚠ ORIENT" : s === "staged_new" ? "\u2B50 NEW WATCH" : s === "staged_watching" ? "\uD83D\uDC41 WATCHING" : s === "staged_confirming" ? "\u23F3 CONFIRMING" : s === "staged_invalidated" ? "\u274C INVALIDATED" : s?.startsWith("skipped_") ? "SKIPPED" : s?.toUpperCase() || "\u2014";
