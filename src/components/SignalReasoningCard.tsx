...
export function SignalReasoningCard({ signalReason, compact = false }: SignalReasoningCardProps) {
  ...
  // Separate enabled from disabled factors (weight=0 OR detail contains "disabled")
  const isFactorDisabled = (f: FactorScore) => f.weight === 0 || (typeof f.detail === "string" && /disabled/i.test(f.detail));
  const enabledScores = factorScores ? factorScores.filter(f => !isFactorDisabled(f) || f.name === "Power of 3 Combo") : null;
  const disabledScores = factorScores ? factorScores.filter(f => isFactorDisabled(f) && f.name !== "Power of 3 Combo") : [];
        ...
        <div>
          ...
          {disabledScores.length > 0 && (
            <div className="mt-2">
              ...
              <p className="text-[8px] uppercase tracking-wider text-muted-foreground/60 mb-1 font-medium">Disabled ({disabledScores.length})</p>
