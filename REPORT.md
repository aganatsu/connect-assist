...
6. **Structural conviction gate tightened** (Gate 3 in bot-scanner). Previously only blocked when supporting direction rate = 0%. Now also blocks when opposing rate > 2.5x supporting rate (e.g., Bear 50% vs Bull 20% = 2.5x -> blocked). This would have blocked the XAG/USD trade from the original analysis.

7. **Conflict counter introduced**. When 4+ factors have opposing (negative) weights, the minimum confluence threshold is raised by 10 percentage points. When 6+ factors oppose, the trade is blocked entirely regardless of score. This applies to both initial entry and staged setup promotion.

8. **Tiered scoring now includes `opposingFactorCount`** in its output, and the summary string includes an `[N opposing]` annotation when opposing factors are present.

## Files modified

- `supabase/functions/_shared/confluenceScoring.ts` — Factor 8 (Reversal Candle), Factor 10 (Displacement), Factor 17 (AMD Phase), Factor 19 (Confluence Stack), Factor 22 (Daily Bias): added bidirectional penalty logic. Tiered scoring loop: added opposing factor handling and conflict counter. Return object: added `opposingFactorCount` to `tieredScoring`.
- `supabase/functions/bot-scanner/index.ts` — Gate 3 (structural conviction): added 2.5x ratio block. Conflict counter: added threshold adjustment and 6+ block logic at both initial entry and staged setup promotion checkpoints.
...
- `supabase/functions/_shared/bidirectionalScoring.test.ts` — New test file (12 tests).
...
4. **Factor 19 Confluence Stack**: Counter-directional stacks now produce negative weights (-0.375 for triple, -0.25 for double, -0.125 for single) instead of reduced positive weights.

5. **Factor 22 Daily Bias**: Present check changed to `pts !== 0` (was already capable of negative scoring).

6. **Tiered scoring loop**: Added `opposingFactorCount` tracking. Factors with `_opposing: true` and `weight < 0` are counted and subtract 50% of tier points from the tiered score.

7. **All 5 factor push lines**: `weight` field now uses `pts < 0 ? -Math.abs(s.displayWeight) : s.displayWeight` so the tiered scoring loop can see negative weights.
...
684 passed | 2 failed (10s)
```

The 2 failures are pre-existing and unrelated:
- `src/test/example.test.ts` — template test (uncaught error)
- `findImpulseLeg — ETH-like bearish impulse` — pre-existing impulse engine test
...
5. **All 684 non-pre-existing tests pass**: No regressions introduced.

## Open questions

1. **XAG/USD trade retrocheck**: The trade from the original analysis (Bull 20% / Bear 50% structural conviction) would now be blocked by the tightened Gate 3 (2.5x ratio). Is this the desired outcome?
2. **Conflict counter thresholds**: 4+ opposing = +10% threshold, 6+ = block. Are these thresholds appropriate, or should they be configurable?
3. **FOTSI (Factor 18)**: Already had negative scoring before this change. Should its negative values also count toward the `opposingFactorCount`? Currently they don't (only factors with `_opposing: true` are counted).

## Suggested PR title and description

**Title:** feat: bidirectional factor scoring with conflict counter

**Description:**
Implements bidirectional scoring for 5 confluence factors (Displacement, Reversal Candle, AMD Phase, Confluence Stack, Daily Bias) so that opposing signals penalize at 50% of max weight instead of scoring 0. Adds a conflict counter that raises the minimum confluence threshold by 10pp when 4+ factors oppose, and blocks entirely when 6+ oppose. Tightens the structural conviction gate to block when opposing rate > 2.5x supporting rate.
...
BEHAVIOR CHANGES: Yes — trades with opposing signals will score lower. Trades with structural conviction ratio > 2.5x opposing will be blocked. Trades with 6+ opposing factors will be blocked.
