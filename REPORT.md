...
- `supabase/migrations/20260509130001_add_kv_cache_cleanup_cron.sql` — NEW. Hourly pg_cron job that deletes expired rows from `kv_cache`.
...
## Open questions
1. The `kv_cache` table migration (`20260509130000`) has been applied to production.
2. The cleanup cron migration (`20260509130001`) needs to be applied to production Supabase.
...
**Migrations required:**
- `20260509130000_create_kv_cache_table.sql` — creates the `kv_cache` table (already applied)
- `20260509130001_add_kv_cache_cleanup_cron.sql` — hourly pg_cron cleanup of expired rows
