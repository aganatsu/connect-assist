...
import { deriveConfirmationTriggerPlan, type ConfirmationBuildDiagnostic, type ConfirmationTriggerPlan } from "./impulseConfirmationLock.ts";
import {
...
export interface TradeLifecycleStepResult {
  ...
  confirmationBuildDiagnostic: ConfirmationBuildDiagnostic | null;
...
export function advanceTradeLifecycle(input: {
  ...
  const diagnosticSink: { current: ConfirmationBuildDiagnostic | null } = { current: null };
  ...
  if (after.status !== "active") return { contractVersion: TRADE_LIFECYCLE_AUTHORITY_VERSION, before, after, events, confirmationPlan, confirmationBuildDiagnostic: diagnosticSink.current, disposition: after.status === "entered" ? "entry_ready" : "terminal" };
