export const CANONICAL_SCANNER_STATE_VERSION = "canonical-scanner-state.v1";
...
export function projectCanonicalScannerState(input: CanonicalScannerStateInput): CanonicalScannerState {
  ...
  if (input.lifecycle?.positionClosed) {
    ...
    stage = "discovery"; reasonCode = "impulse_zone_pending"; explanation = "Direction is available; searching for a qualified impulse-owned POI";
  } else if (input.zone.valid === false) {
    stage = "invalidated"; reasonCode = input.zone.reasonCode || "zone_invalid"; explanation = "The impulse-owned zone is no longer valid";
  ...
  const authorities: ScannerAuthorityReference[] = [
    ...
    { role: "impulse_zone", source: input.zone.source || "unified_zone", available: input.zone.available, passed: input.zone.valid, evidenceId: input.zone.evidenceId, reasonCode: input.zone.reasonCode },
