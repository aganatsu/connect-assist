...
/**
 * Replicates Gate 9b logic. Returns null when gate is disabled (no entry pushed).
 */
function smtVetoGate(factors: Factor[] | undefined | null, enabled: boolean = true): GateResult | null {
  if (enabled === false) return null; // gate disabled, no entry
...
// ─── Test 6: Config toggle — disabled means gate does not fire ──
Deno.test("Gate 9b: smtOppositeVeto = false → gate skipped entirely", () => {
  const factors: Factor[] = [
    { name: "SMT Divergence", present: false, weight: 0, detail: "SMT detected (bearish) but opposite to signal direction" },
  ];

  // With enabled = false, the gate should not produce any entry
  const result = smtVetoGate(factors, false);
  assertEquals(result, null, "Gate should return null (skipped) when config disabled");

  // With enabled = true (default), it should veto
  const resultEnabled = smtVetoGate(factors, true);
  assertEquals(resultEnabled?.passed, false, "Gate should FAIL when enabled and SMT opposes");
});
...
Deno.test("Gate 9b: Source code presence verification in bot-scanner/index.ts", () => {
  ...
  // Verify config toggle is respected
  const hasConfigCheck = source.includes("config.smtOppositeVeto !== false");
  assertEquals(hasConfigCheck, true, "Gate must check config.smtOppositeVeto toggle");
});
