...
import { calculateRoundTripCommission } from "../../functions/_shared/tradingCosts.ts";
...
Deno.test("Commission: round-trip input is applied once", () => {
  const lots = 0.5;
  const commissionPerLot = 7; // $7 per lot round trip
  const roundTripCommission = calculateRoundTripCommission(
    lots,
    commissionPerLot,
  );
  assertEquals(roundTripCommission, 3.50);
});
...
Deno.test("Commission: partial TP commission is proportional to closed size", () => {
  ...
  const partialCommission = calculateRoundTripCommission(
    closedSize,
    commissionPerLot,
  );
  assertEquals(partialCommission, 3.50);
});
