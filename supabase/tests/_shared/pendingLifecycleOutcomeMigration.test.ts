...
Deno.test("pending lifecycle identity survives every paper trade close writer", () => {
  ...
  const historyWrites =
    source.match(/from\("paper_trade_history"\)\.insert/g) || [];
  const identityWrites = source.match(/source_pending_order_id:/g) || [];
  // Partial TP is the only non-final history writer. Terminal closes delegate
  // to the atomic database finalizer, which copies the pending identity from
  // the locked source position.
  assertEquals(historyWrites.length, 1);
  assertEquals(identityWrites.length, historyWrites.length);
  const finalizer = Deno.readTextFileSync(
    "supabase/migrations/20260817110000_atomic_paper_position_close.sql",
  );
  assert(finalizer.includes("v_position.source_pending_order_id"));
});
...
Deno.test("pending lifecycle outcome migration adds an indexed history link", () => {
  ...
  assert(
    migration.includes("ADD COLUMN IF NOT EXISTS source_pending_order_id UUID"),
  );
  assert(
    migration.includes(
