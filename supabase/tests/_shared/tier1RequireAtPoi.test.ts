 ...
 *
 * The obvious misreading is "require an OB or FVG to exist" — but the
 ...
 *
 * Same 4:1 loose-versus-strict gap as the 2026-09-06 shadow data, where
...
Deno.test("the toggle is in the UI and hidden when the gate itself is off", () => {
  ...
  // Two sibling elements inside `{cond && ( ... )}` need a fragment. Omitting
  // it is a syntax error, not a lint nit — it broke the build on 2026-09-09
  // and the repair moved `)}` above the toggle, silently pushing it outside
  // the conditional so it rendered while Gate 19 was disabled.
  const guard = modal.indexOf("{(config.strategy?.tier1GateEnabled ?? true) && (<>");
  assert(guard > -1, "the conditional must open a fragment");
  const close = modal.indexOf("</>)}", guard);
  const toggle = modal.indexOf("tier1RequireAtPOI", guard);
  assert(close > -1, "and close it");
  assert(toggle > guard && toggle < close, "toggle must render inside the enabled block");
});
