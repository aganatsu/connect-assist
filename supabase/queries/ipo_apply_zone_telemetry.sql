-- APPLY PACK — the zone-telemetry migration, in one transaction with its
-- bookkeeping row, so a failure rolls back both and they can never disagree.
--
-- GENERATED from supabase/migrations/20260922020000_ipo_zone_telemetry.sql.
-- A test asserts the DDL below is byte-identical to that file.
...
values ('20260922020000') on conflict (version) do nothing;

commit;

-- Verify:
--   select column_name from information_schema.columns
--    where table_name='ipo_paper_trade_history'
--      and column_name in ('zone_entry_ordinal','zone_previous_exit_time',
