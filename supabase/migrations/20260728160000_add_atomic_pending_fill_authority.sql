-- Phase 1: make confirmed pending-order fills atomic and idempotent.
--
-- Deployment order:
...
-- The original repository migration omitted awaiting_confirmation from the
-- status constraint. Repair it explicitly so a clean migration replay matches
-- the state machine used by both scanners.
ALTER TABLE public.pending_orders
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_paper_positions_pending_source
  ON public.paper_positions (source_pending_order_id)
