-- URGENT: this trigger has been rejecting every trade-creating insert.
--
...
-- paper_positions AND staged_setups, and validated contractVersion against an
-- allowlist of two setup-policy versions. PR #539 began writing a DIFFERENT
-- record, frozen-decision.v1, into the same column without checking the
-- trigger guarding it — so from the moment it deployed, every qualifying setup
-- and every market entry was refused at the final step. Observed on USD/CHF,
-- USD/JPY, EUR/GBP and NZD/USD; the scanner logs it as
-- zone_setup_insert_failed and moves on, so nothing surfaced as an outage.
--
-- Two contracts now share the column, told apart by contractVersion, which is
-- exactly what that field is for. The setup-policy shape checks are scoped to
-- the setup-policy versions; frozen-decision.v1 carries none of those fields
-- and must not be measured against them.
--
-- Body copied verbatim from 20260914000000_baseline_schema.sql with those two
...
BEGIN
  ...
  -- Shape checks below describe the setup-policy contract ONLY.
  -- frozen-decision.v1 is a different record in the same column,
  -- distinguished by contractVersion, and has none of these fields.
  IF v_version LIKE 'setup-policy-freeze.%' AND (
     jsonb_typeof(v_context->'stylePolicy') IS DISTINCT FROM 'object'
