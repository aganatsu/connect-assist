...
| 5. Confirmation authority | Partial and behaviorally unresolved | `zoneConfirmation.ts` delegates to `confirmationHierarchy.ts`, then can authorize through legacy close/wick/reversal fallbacks when the hierarchy is not entry-ready. | Inventory accepted patterns and represent each as a named level in one versioned contract, observation-first. |
